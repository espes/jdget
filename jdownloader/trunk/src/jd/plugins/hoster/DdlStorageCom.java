...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "ddlstorage.com" }, urls = { "https?://(www\\.)?ddlstorage\\.com/((vid)?embed\\-)?[a-z0-9]{12}" }, flags = { 2 })
...
public class DdlStorageCom extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink downloadLink) throws Exception {
        ...
        if (br.getURL().matches(".+(\\?|&)op=login(.*)?") || cbr.containsHTML("<li>The Premium account in your possession does not allow the download")) {
    ...
    private AccountInfo fetchAccountInfoAPI(Account account, AccountInfo ai) throws Exception {
        ...
        if (usr_disk_space != null) {
            ai.setUsedSpace(SizeFormatter.getSize(usr_disk_space));
        }
    ...
    private void handleDl(final DownloadLink downloadLink, final Account account) throws Exception {
        ...
        if (!inValidate(dllink) && dllink.endsWith("%27") && !downloadLink.getName().endsWith("'")) {
            dllink = dllink.substring(0, dllink.length() - 3);
        }
