...
public class SizeFormatter {
    ...
    public static long getSize(final String string) {
        String[][] matches = new RegularExpression(string, Pattern.compile("([\\d]+)[\\.|\\,|\\:]([\\d]+)", Pattern.CASE_INSENSITIVE)).getMatches();

        if (matches == null || matches.length == 0) {
            matches = new RegularExpression(string, Pattern.compile("([\\d]+)", Pattern.CASE_INSENSITIVE)).getMatches();
        }
        ...
        if (RegularExpression.matches(string, Pattern.compile("(gb|gbyte|gig|gib)", Pattern.CASE_INSENSITIVE))) {
            res *= 1024 * 1024 * 1024;
        } else if (RegularExpression.matches(string, Pattern.compile("(mb|mbyte|megabyte|mib)", Pattern.CASE_INSENSITIVE))) {
            res *= 1024 * 1024;
        } else if (RegularExpression.matches(string, Pattern.compile("(kb|kbyte|kilobyte|kib)", Pattern.CASE_INSENSITIVE))) {
            res *= 1024;
        }
