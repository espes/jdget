...
public class TimeFormatter {
    ...
    public static long formatStringToMilliseconds(String text) {
        String[] found = new AwReg(text, "(\\d+)\\w?:(\\d+)").getRow(0);
    ...
    public static long getMilliSeconds(final String wait) {
        String[][] matches = new AwReg(wait, "([\\d]+) ?[\\.|\\,|\\:] ?([\\d]+)").getMatches();
        if (matches == null || matches.length == 0) {
            matches = new AwReg(wait, Pattern.compile("([\\d]+)")).getMatches();
        }
        ...
        if (org.appwork.utils.AwReg.matches(wait, Pattern.compile("(h|st)", Pattern.CASE_INSENSITIVE))) {
            res *= 60 * 60 * 1000l;
        } else if (org.appwork.utils.AwReg.matches(wait, Pattern.compile("(m)", Pattern.CASE_INSENSITIVE))) {
            res *= 60 * 1000l;
        } else {
            res *= 1000l;
        }
