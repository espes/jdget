...
public class HTTPConnectionUtils {
	...
	public static ByteBuffer readheader(final InputStream in,
			final boolean onlyHTTPHeader) throws IOException {
		...
		while ((c = in.read(minibuffer)) >= 0) {
			if (bigbuffer.remaining() < 1) {
				final ByteBuffer newbuffer = ByteBuffer.allocate((bigbuffer
						.capacity() * 2));
				bigbuffer.flip();
			...
			if (onlyHTTPHeader) {
				...
				if (bigbuffer.position() >= 2) {
					...
					position = bigbuffer.position();
					if (bigbuffer.get(position - 2) == HTTPConnectionUtils.R
							&& bigbuffer.get(position - 1) == HTTPConnectionUtils.N) {
						break;
					}
				}
			} else {
