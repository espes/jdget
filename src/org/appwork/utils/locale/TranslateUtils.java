...
public class TranslateUtils {
    ...
    public static String checkTranslateFiles(final SourceParser sourceParser, final Class<?> clazz, final boolean write) throws Exception {
        ...
        final AwReg reg = new AwReg(source, "(.*?\\{)");
        final String pre = reg.getMatch(0);
        final String post = new AwReg(source, "(//\\s*ENDOFENUMS.*)").getMatch(0);
        ...
        for (final Field f : clazz.getDeclaredFields()) {
            if (Modifier.isPublic(f.getModifiers()) && Modifier.isStatic(f.getModifiers()) && Modifier.isFinal(f.getModifiers())) {
                ...
                fin.append("\r\n");
                String line = new AwReg(source, f.getName() + "\\s*\\(\"[^\r^\n]*?(?<!\\\\)\"\\, \\d+\\)[\\,\\;]").getMatch(-1);

                if (line == null) {
                    line = new AwReg(source, f.getName() + "\\s*\\(\"[^\r^\n]*?(?<!\\\\)\"\\)[\\,\\;]").getMatch(-1);
                }
                if (line.contains("CHATPANEL_HISTORY_LASTWEEK")) {
                    System.out.println("fdsf");
                }
                String comment = new AwReg(source, "\\/\\*(.*?)\\*\\/\\s*" + Pattern.quote(line)).getMatch(0);
                if (comment != null) {
                    if (comment.contains("/*")) {
                    ...
                    fin.append("\r\n*/\r\n");
                    final String full = new AwReg(source, "(\\/\\*.*?\\*\\/\\s*" + Pattern.quote(line) + ")").getMatch(0);
    ...
    public static String createLocFile(final String lng, final File[] files, final Class<?>[] classes) throws Exception {
        ...
        for (int i = 0; i < classes.length; i++) {
            ...
            String source = IO.readFileToString(file);
            source = new AwReg(source, "public\\s+enum.*?implements\\s+Translate\\s+\\{(.*)").getMatch(0);
            ...
            for (final Translate entry : values) {
                ...
                String line = new AwReg(source, name + "\\s*\\(\"[^\r^\n]*?(?<!\\\\)\"\\, \\d+\\)[\\,\\;]").getMatch(-1);
                System.out.println(name);

                if (line == null) {
                    line = new AwReg(source, name + "\\s*\\(\"[^\r^\n]*?(?<!\\\\)\"\\)[\\,\\;]").getMatch(-1);
                }
                if (line == null) {
                ...
                if (comment != null) {
                    ...
                    comment = new AwReg(comment, "###" + lng + ":([^\r^\n]+)").getMatch(0);
                    if (comment != null) {
                        comment = comment.split("###")[0].trim();
                    }
                    source = source.replace(new AwReg(source, "(\\/\\*.*?\\*\\/\\s*" + Pattern.quote(line) + ")").getMatch(0), "---\r\n" + line);
                }
    ...
    private static String handleMultiComment(String comment) {
        comment = new AwReg(comment, ".*\\/\\*(.*)").getMatch(0);
        return comment;
    }
}
